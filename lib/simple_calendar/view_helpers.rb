            ...
            content_tag(:span, :class => (week.include?(today) ? "current-week week" : "week")) do
              ...
              week.collect do |date|
                ...
                content_tag(:span, :class => day_class.join(" "), :'data-date-iso'=>date.to_s, 'data-date'=>date.to_s.gsub('-', '/')) do
                    spans = []
                    concat content_tag(:a, date.day.to_s, :href=>"/performances/search?year=#{(params[:year].blank?) ? Time.now.year : params[:year]}&month=#{(params[:month].blank?) ? Time.new.month : params[:month] }&city=#{params[:city]}&title=#{params[:title]}&day=#{date.day.to_s}", :class=>"day_number")
